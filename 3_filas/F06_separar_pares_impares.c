/*
 * ==========================================================================
 *  F06 — FILAS | Separar pares e ímpares
 * --------------------------------------------------------------------------
 *  DESAFIO
 *  Percorra a fila 'f' e ENFILEIRE cada número par em 'pares' e cada número
 *  ímpar em 'impares', preservando a ordem de chegada.
 *
 *  - A fila 'f' NÃO pode ser alterada (ao final, mesmo conteúdo e ordem).
 *  - 'pares' e 'impares' já chegam criadas e vazias.
 *  - Números negativos também contam (-4 é par, -3 é ímpar).
 *    CUIDADO: em C, -3 % 2 == -1 (e não 1)!
 *  - Se algum ponteiro for NULL, não faça nada.
 *
 *  Exemplo: f = [5, 2, 8, 3, 1, 4] -> pares = [2, 8, 4], impares = [5, 3, 1]
 *
 *  ASSINATURA
 *      void separarParesImpares(Fila *f, Fila *pares, Fila *impares);
 *
 *  COMO TESTAR
 *      ./testar.sh F06
 * ==========================================================================
...
void caso(const int *v, int n, const int *ep, int np, const int *ei, int ni, const char *desc) {
    secao(desc);
    Fila *f = filaDe(v, n), *p = filaDe(NULL, 0), *i = filaDe(NULL, 0);
    separarParesImpares(f, p, i);
    checarFila(p, ep, np, "pares");
    checarFila(i, ei, ni, "ímpares");
    checarFila(f, v, n, "fila original intacta");
    liberarFilaTeste(f);
    liberarFilaTeste(p);
    liberarFilaTeste(i);
}
...
int main(void) {
    printf("F06 — separarParesImpares\n");

    caso((int[]){5, 2, 8, 3, 1, 4}, 6, (int[]){2, 8, 4}, 3, (int[]){5, 3, 1}, 3, "[5, 2, 8, 3, 1, 4]");
    caso((int[]){2, 4, 6}, 3, (int[]){2, 4, 6}, 3, NULL, 0, "só pares [2, 4, 6]");
    caso((int[]){1, 3}, 2, NULL, 0, (int[]){1, 3}, 2, "só ímpares [1, 3]");
    caso(NULL, 0, NULL, 0, NULL, 0, "fila vazia");
    caso((int[]){-3, 0, -4, 7}, 4, (int[]){0, -4}, 2, (int[]){-3, 7}, 2, "com negativos e zero [-3, 0, -4, 7]");

    secao("Ponteiros NULL");
    Fila *f = filaDe((int[]){1, 2}, 2), *p = filaDe(NULL, 0);
    separarParesImpares(f, p, NULL);
    separarParesImpares(NULL, p, p);
    verificar(1, "não quebra com ponteiros NULL");
    checarFila(f, (int[]){1, 2}, 2, "fila original intacta");
    liberarFilaTeste(f);
    liberarFilaTeste(p);
