/*
 * ==========================================================================
 *  F09 — FILAS | Atendimento prioritário (mover para a frente)
 * --------------------------------------------------------------------------
 *  DESAFIO
 *  Mova a PRIMEIRA ocorrência de 'valor' para o INÍCIO da fila, mantendo
 *  a ordem relativa dos demais. Retorne 1 se encontrou, 0 caso contrário
 *  (fila inalterada).
 *
 *  Exemplos (início -> fim):
 *    [1, 2, 3, 4], valor 3  ->  [3, 1, 2, 4]
 *    [1, 2, 3, 4], valor 4  ->  [4, 1, 2, 3]   (o 'fim' muda!)
 *    [1, 2, 3, 4], valor 1  ->  [1, 2, 3, 4]
 *    [5, 3, 5],    valor 5  ->  [5, 3, 5]      (só a 1ª ocorrência)
 *
 *  Pode resolver manipulando os ponteiros (inicio/fim/prox) OU usando só
 *  enfileirar/desenfileirar + uma fila auxiliar. O teste confere inicio,
 *  fim e tamanho.
 *  f == NULL -> retorna 0.
 *
 *  ASSINATURA
 *      int moverParaFrente(Fila *f, int valor);
 *
 *  COMO TESTAR
 *      ./testar.sh F09
 * ==========================================================================
...
void caso(const int *v, int n, int valor, const int *esp, int retorno, const char *desc) {
    secao(desc);
    Fila *f = filaDe(v, n);
    int r = moverParaFrente(f, valor);
    checarFila(f, esp, n, "fila resultante");
    char d[60];
    snprintf(d, sizeof d, "retorna %d", retorno);
    if (!verificar(r == retorno, d)) printf("      obtido: %d\n", r);
    liberarFilaTeste(f);
}

int main(void) {
    printf("F09 — moverParaFrente\n");

    secao("NULL");
    verificar(moverParaFrente(NULL, 1) == 0, "moverParaFrente(NULL, 1) retorna 0");

    caso((int[]){1, 2, 3, 4}, 4, 3, (int[]){3, 1, 2, 4}, 1, "[1, 2, 3, 4] mover 3");
    caso((int[]){1, 2, 3, 4}, 4, 4, (int[]){4, 1, 2, 3}, 1, "[1, 2, 3, 4] mover 4 (último)");
    caso((int[]){1, 2, 3, 4}, 4, 1, (int[]){1, 2, 3, 4}, 1, "[1, 2, 3, 4] mover 1 (já é o primeiro)");
    caso((int[]){1, 2, 3, 4}, 4, 9, (int[]){1, 2, 3, 4}, 0, "[1, 2, 3, 4] mover 9 (não existe)");
    caso((int[]){5, 3, 5}, 3, 5, (int[]){5, 3, 5}, 1, "[5, 3, 5] mover 5");
    caso((int[]){2, 8, 6, 8}, 4, 8, (int[]){8, 2, 6, 8}, 1, "[2, 8, 6, 8] mover 8 (só a 1ª)");
    caso((int[]){7}, 1, 7, (int[]){7}, 1, "[7] mover 7");
    caso(NULL, 0, 7, NULL, 0, "[] mover 7");

    secao("A fila continua funcionando depois");
    Fila *f = filaDe((int[]){1, 2, 3}, 3);
    moverParaFrente(f, 3);
    enfileirar(f, 4);
    checarFila(f, (int[]){3, 1, 2, 4}, 4, "mover 3 e enfileirar 4 -> [3, 1, 2, 4]");
    liberarFilaTeste(f);
