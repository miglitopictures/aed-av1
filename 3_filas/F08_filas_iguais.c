/*
 * ==========================================================================
 *  F08 — FILAS | Verificar se duas filas são iguais
 * --------------------------------------------------------------------------
 *  DESAFIO
 *  Retorne 1 se as duas filas têm os mesmos elementos na mesma ordem
 *  (mesmo tamanho inclusive) e 0 caso contrário.
 *
 *  - Use apenas enfileirar / desenfileirar (+ campo tamanho).
 *  - As filas NÃO podem ser alteradas: ao final, as duas devem estar
 *    exatamente como chegaram — mesmo quando a resposta for 0!
 *    (Não pare no meio do "giro" sem recolocar tudo.)
 *  - Duas filas vazias são iguais. Se algum ponteiro for NULL, retorne 0.
 *
 *  ASSINATURA
...
void caso(const int *a, int na, const int *b, int nb, int esperado, const char *desc) {
    secao(desc);
    Fila *F1 = filaDe(a, na), *F2 = filaDe(b, nb);
    int r = filasIguais(F1, F2);
    char d[60];
    snprintf(d, sizeof d, "retorna %d", esperado);
    if (!verificar(r == esperado, d)) printf("      obtido: %d\n", r);
    checarFila(F1, a, na, "F1 intacta");
    checarFila(F2, b, nb, "F2 intacta");
    liberarFilaTeste(F1);
    liberarFilaTeste(F2);
}

int main(void) {
    printf("F08 — filasIguais\n");

    caso((int[]){1, 2, 3}, 3, (int[]){1, 2, 3}, 3, 1, "[1, 2, 3] e [1, 2, 3]");
    caso((int[]){1, 2, 3}, 3, (int[]){1, 5, 3}, 3, 0, "[1, 2, 3] e [1, 5, 3] (diferem no meio)");
    caso((int[]){1, 2, 3}, 3, (int[]){3, 2, 1}, 3, 0, "[1, 2, 3] e [3, 2, 1] (ordem diferente)");
    caso((int[]){1, 2}, 2, (int[]){1, 2, 3}, 3, 0, "[1, 2] e [1, 2, 3] (tamanhos diferentes)");
    caso(NULL, 0, NULL, 0, 1, "[] e [] (ambas vazias)");
    caso(NULL, 0, (int[]){4}, 1, 0, "[] e [4]");
    caso((int[]){9, 1}, 2, (int[]){8, 1}, 2, 0, "[9, 1] e [8, 1] (diferem no primeiro)");

    secao("Ponteiros NULL");
    Fila *f = filaDe((int[]){1}, 1);
    verificar(filasIguais(f, NULL) == 0 && filasIguais(NULL, f) == 0, "com NULL retorna 0");
    liberarFilaTeste(f);
