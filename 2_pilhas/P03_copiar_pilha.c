/*
 * ==========================================================================
 *  P03 — PILHAS | Copiar uma pilha usando só as operações da pilha
 * --------------------------------------------------------------------------
 *  DESAFIO
 *  Crie e retorne uma NOVA pilha com os mesmos elementos, na MESMA ordem
 *  (mesmo topo), da pilha recebida.
 *
 *  - Ao final, a pilha original deve estar exatamente como antes.
 *  - Use apenas criarPilha / empilhar / desempilhar / pilhaVazia /
 *    liberarPilha (não mexa diretamente em ->topo ou ->prox).
 *    Dica: você vai precisar de uma pilha auxiliar.
 *  - Pilha original vazia -> retorne uma pilha nova e vazia (não NULL).
 *  - Se p == NULL, retorne NULL.
 *
 *  ASSINATURA
...
void caso(const int *v, int n, const char *desc) {
    secao(desc);
    Pilha *p = pilhaDe(v, n);
    Pilha *c = copiarPilha(p);
    checarPilha(c, v, n, "a cópia tem os mesmos elementos");
    checarPilha(p, v, n, "a original não mudou");
    if (c != NULL) {
        verificar(c != p && !pilhasCompartilham(c, p), "a cópia é independente (struct e nós novos)");
        if (c != p && !pilhasCompartilham(c, p)) liberarPilhaTeste(c);
    }
    liberarPilhaTeste(p);
}

int main(void) {
    printf("P03 — copiarPilha\n");

    secao("p == NULL");
    verificar(copiarPilha(NULL) == NULL, "copiarPilha(NULL) retorna NULL");

    caso((int[]){1, 2, 3, 4}, 4, "Pilha [1, 2, 3, 4]");
    caso(NULL, 0, "Pilha vazia");
    caso((int[]){42}, 1, "Pilha [42]");
    caso((int[]){5, 5, 1, 9, 5}, 5, "Pilha com repetidos [5, 5, 1, 9, 5]");

    secao("Independência");
    Pilha *p = pilhaDe((int[]){7, 8}, 2);
    Pilha *c = copiarPilha(p);
    if (c != NULL && c != p && !pilhasCompartilham(c, p)) {
        empilhar(c, 99);
        checarPilha(p, (int[]){7, 8}, 2, "empilhar na cópia não afeta a original");
        liberarPilhaTeste(c);
    } else {
        verificar(0, "empilhar na cópia não afeta a original");
    }
    liberarPilhaTeste(p);
