/*
 * ==========================================================================
 *  A09 — ABB | Questão de papel                          (Simulado 2 — Q05)
 * --------------------------------------------------------------------------
 *  DESAFIO  (faça TUDO no papel antes de preencher!)
 *  Considere uma árvore binária de busca inicialmente vazia.
 *
 *  a) Insira os valores, nesta ordem:
 *       60, 35, 80, 20, 45, 70, 90, 10, 30, 40, 50, 65, 75, 85, 95
 *     (itens extras de treino: escreva também a EM-ORDEM e a PÓS-ORDEM)
 *
 *  Em seguida, faça as remoções abaixo, redesenhando a árvore após cada
 *  uma (nó com dois filhos: use o MENOR valor da subárvore DIREITA):
 *  b) Remova o nó 10
 *  c) Remova o nó 20
 *  d) Remova o nó 35
 *
 *  COMO CONFERIR OS DESENHOS
 *  Escreva a PRÉ-ORDEM de cada árvore que você desenhou (ela determina a
 *  forma de uma ABB).
 *
 *  COMO TESTAR
 *      ./testar.sh A09
...
/* >>>>>>>>>>>>>>>>>>>>>>>>>> SUA SOLUÇÃO AQUI >>>>>>>>>>>>>>>>>>>>>>>>>> */
int a_preOrdem[]   = { 0 };   /* desenho do item a */
int a_emOrdem[]    = { 0 };   /* extra */
int a_posOrdem[]   = { 0 };   /* extra */
int b_preOrdem[]   = { 0 };   /* após remover 10 */
int c_preOrdem[]   = { 0 };   /* após remover 20 */
int d_preOrdem[]   = { 0 };   /* após remover 35 */
/* <<<<<<<<<<<<<<<<<<<<<<<<<< FIM DA SUA SOLUÇÃO <<<<<<<<<<<<<<<<<<<<<<<<<< */
...
int main(void) {
    printf("A09 — ABB no papel (Simulado 2)\n");
    int ins[] = {60, 35, 80, 20, 45, 70, 90, 10, 30, 40, 50, 65, 75, 85, 95};
    NoArvore *r = abbDe(ins, TAM(ins));

    secao("a) Construção");
    conferirPre(r, a_preOrdem, TAM(a_preOrdem), "a) pré-ordem da árvore construída");
    conferirEm(r, a_emOrdem, TAM(a_emOrdem), "a) em-ordem (extra)");
    conferirPos(r, a_posOrdem, TAM(a_posOrdem), "a) pós-ordem (extra)");

    secao("Remoções");
    r = removerRef(r, 10);
    conferirPre(r, b_preOrdem, TAM(b_preOrdem), "b) após remover 10");
    r = removerRef(r, 20);
    conferirPre(r, c_preOrdem, TAM(c_preOrdem), "c) após remover 20");
    r = removerRef(r, 35);
