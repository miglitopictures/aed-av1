/*
 * ==========================================================================
 *  A03 — ÁRVORES | Contar nós com exatamente um filho
 *                                                   (Simulado 2026.2 — Q03)
 * --------------------------------------------------------------------------
 *  DESAFIO
 *  Crie uma função RECURSIVA que conte quantos nós de uma árvore binária
 *  possuem EXATAMENTE UM filho. Folhas e nós com dois filhos não devem ser
 *  contabilizados. (A árvore não precisa ser ABB.)
 *
 *  ASSINATURA
 *      int contarNosComUmFilho(NoArvore *raiz);
 *
 *  COMO TESTAR
 *      ./testar.sh A03
 * ==========================================================================
...
void caso(NoArvore *r, int esperado, const char *desc) {
    int obtido = contarNosComUmFilho(r);
    char d[200];
...
int main(void) {
    printf("A03 — contarNosComUmFilho\n");

    secao("Casos base");
    caso(NULL, 0, "árvore vazia");
    caso(N(1, NULL, NULL), 0, "só a raiz");
    caso(N(1, N(0, NULL, NULL), NULL), 1, "raiz com só filho esquerdo");
    caso(N(1, NULL, N(2, NULL, NULL)), 1, "raiz com só filho direito");

    secao("Árvores maiores");
    caso(arvoreCompleta7(), 0, "árvore completa de 7 nós (todos têm 0 ou 2 filhos)");
    caso(arvoreSimulado2(), 1, "árvore do Simulado 2 (só o 15)");
    caso(arvoreTorta(), 3, "árvore torta (5, 3 e 20)");
    caso(arvoreCorrente(), 3, "corrente 1->2->3->4 (1, 2 e 3)");
